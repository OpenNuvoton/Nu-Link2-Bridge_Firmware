...
void VCOM_TransferData(void)
{
    ...
    if (monRshorts) {
        ...
        if (u8SlaveAddr & BIT7) { // i2c read
            ...
            if (1 == u16rwLen) {
                data[0] = UI2C_ReadByte(UI2C0, u8SlaveAddr);
                comRbytes = 1;
            } else {
                comRbytes = UI2C_ReadMultiBytes(UI2C0, u8SlaveAddr, data, u16rwLen);
            }
        } else { // i2c write
            ...
            if (1 == u16rwLen) {
                if (0 == UI2C_WriteByte(UI2C0, u8SlaveAddr, data[0])) {
                    comRbytes = 1;
                } else {
                    comRbytes = 0;
                }
            } else {
                comRbytes = UI2C_WriteMultiBytes(UI2C0, u8SlaveAddr, data, u16rwLen);
            }
        }
